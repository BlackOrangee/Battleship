...
// This function removes the "bang" symbol from the game field 
// and replaces it with the "point" symbol.
...
// It checks the specified coordinates on the unvisible field
...
// 2 -- invalid move.
//
int Fire(char** field_unvisible, char** field_visible, int& size, 
	int* input, int input_size)
{
	...
	{
		// If the coordinate is empty, mark it as a hit (bang)
		// on both the unvisible and visible fields
		field_unvisible[input[0]][input[1]] = bang;
		field_visible[input[0]][input[1]] = bang;
		return 0; // Return 0 to indicate a miss
	}
	else if (field_unvisible[input[0]][input[1]] == point 
		|| field_unvisible[input[0]][input[1]] == cross)
	{
		// If the coordinate has already been targeted (either hit or missed),
		// return 2 to indicate an invalid move
		return 2;
	}
	else if (field_unvisible[input[0]][input[1]] == ship)
	{
		// If the coordinate contains a ship, mark it as a hit (cross)
		// on both the unvisible and visible fields
		field_unvisible[input[0]][input[1]] = cross;
...
void Check_Ship_Vertical(char** field_2_unvisible, int& size, 
	Ship* ships, int& ships_count, bool* on_watter, int& k)
{
...
void Check_Ship_Horizontal(char** field_2_unvisible, int& size, 
	Ship* ships, int& ships_count, bool* on_watter, int& k)
{
	// Iterate over the ship's length
	for (int i = 0; i < ships[k].ship_size; i++)
...
// Check the ship's size and the on_watter values 
// to determine if the ship is still on the water
...
void Check_Ship(char** field_2_unvisible, int& size, 
	Ship* ships, int& ships_count)
{
	for (int k = 0; k < ships_count; k++)
	{
		...
		if (ships[k].orient == 1)
		{
			Check_Ship_Vertical(field_2_unvisible, size, 
				ships, ships_count, on_watter, k); // Call the helper function for vertical ships
		}
		else if (ships[k].orient == 2)
		{
			Check_Ship_Horizontal(field_2_unvisible, size, 
				ships, ships_count, on_watter, k); // Call the helper function for horizontal ships
		}
...
// This function updates the unvisible and visible fields 
// by marking the surrounding area of a vertically oriented ship with points.
void Pointer_Vertical(char** field_1_visible, char** field_2_unvisible, int& size, 
	Ship* ships, int& ships_count, int& k)
{
...
// This function updates the unvisible and visible fields 
// by marking the surrounding area of a horizontally oriented ship with points.
void Pointer_Horizontal(char** field_1_visible, char** field_2_unvisible, int& size, 
	Ship* ships, int& ships_count, int& k)
{
	for (int i = 0; i < ships[k].ship_size + 2; i++)
	{
...
// This function updates the unvisible and visible fields 
// by marking the surrounding area of ships with points.
void Pointer(char** field_1_visible, char** field_2_unvisible, 
	int& size, Ship* ships, int& ships_count)
{
	for (int k = 0; k < ships_count; k++)
	{
		...
		if (ships[k].orient == 1)
		{
			Pointer_Vertical(field_1_visible, field_2_unvisible, size, 
				ships, ships_count, k); // Call the helper function for vertical ships
		}
		else if (ships[k].orient == 2)
		{
			Pointer_Horizontal(field_1_visible, field_2_unvisible, size, 
				ships, ships_count, k); // Call the helper function for horizontal ships
		}
	}
}
...
int Score_check(int& score)
{
	if (score == 20)
	{
		return 0; // Return 0 if the score reaches 20 -- Win
	}
...
void PC_Turn(char** field_1_Player, char** field_2_Enemy, char** field_Enemy_Memory, 
	int& size, Ship* Players_Ships, Ship* PC_Ships, int& ship_count)
{
	int pc_input_size = 2;
	...
	do
	{
		...
		check = PC_AI_Check_Ship(field_Enemy_Memory, size, 
			Players_Ships, ship_count, pc_input, pc_input_size); // Generate the PC's input coordinates

		if (tactics)
		{
			if (tactic_1 && !tactic_2 && !check)
			{
				check = PC_Start_Tactic_1(field_Enemy_Memory, size, pc_input, pc_input_size);
			}
			else if (!tactic_2 && !check)
			{
				check = PC_Start_Tactic_2(field_Enemy_Memory, size, pc_input, pc_input_size);
			}
			else if (tactic_2 && !check)
			{
				if (tactic_first && !check)
				{
					check = PC_Start_Tactic_1(field_Enemy_Memory, size, pc_input, pc_input_size);
					if (!check) { tactic_first = false; }
				}
				else
				{
					check = PC_Start_Tactic_2(field_Enemy_Memory, size, pc_input, pc_input_size);
					if (!check) { tactic_first = true; }
				}
			}
		...
		fire = Fire(field_1_Player, field_Enemy_Memory, size, 
			pc_input, pc_input_size); // Fire at the player's field using the generated input
...
void PC_2_Turn(char** field_Enemy, char** field_Enemy_Memory, char** field_2_Enemy, 
	char** field_1_Player, int& size, Ship* PC_Ships, Ship* Players_Ships, int& ship_count)
{
	int pc_input_size = 2;
	int* pc_input = new int[pc_input_size];
	int fire = 0;
	...
	do
	{
		...
		check = PC_AI_Check_Ship(field_2_Enemy, size, PC_Ships, ship_count,
			pc_input, pc_input_size); // Generate the PC's input coordinates
		
		if (tactic_pc_2 && !check)
		{
			check = PC_Start_Tactic_1(field_2_Enemy, size, pc_input, pc_input_size);
		...
		{
			check = PC_Start_Tactic_2(field_2_Enemy, size, pc_input, pc_input_size);
			if (pc_input[0] == 0 && pc_input[1] == 0)
			{
				tactic_pc_2 = true;
			}
		...
		fire = Fire(field_Enemy, field_2_Enemy, size, 
			pc_input, pc_input_size); // Fire at the player's field using the generated input
...
void Player_Turn(char** field_1_Player, char** field_2_Enemy, char** field_Enemy, 
	int& size, Ship* Players_Ships, Ship* PC_Ships, int& ship_count)
{
	...
	do
	{
		...
		fire = Fire(field_Enemy, field_2_Enemy, size, 
			input, input_size); // Fire at the enemy's field using the player's input
